...
namespace Uniplac.Lavacao.Testes.Twitter
{
    [TestClass]
    public class TwitterServiceTeste
    {
        ...
        {
            var service = GetAuthenticatedService();
            var status = "Teste com Localização: " + DateTime.UtcNow.Ticks;
        ...
        {
            var service = GetAuthenticatedService();
            using (var stream = new FileStream("abstract.png", FileMode.Open))
            {
                var tweet = service.SendTweetWithMedia(new SendTweetWithMediaOptions
                {
                    Status = "Teste com imagem: " + DateTime.UtcNow.Ticks,
        ...
        public void Can_exchange_for_access_token()
        {
            ...
            var verifier = "4204697"; // <-- Debugger breakpoint and edit with the actual verifier
